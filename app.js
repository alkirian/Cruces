...
function renderParticipants() {
  elements.mcList.innerHTML = state.participants
    .map((name, index) => {
        ...
        for (let triWinners = 1; triWinners <= 2; triWinners++) {
          for (let numTri = 0; numTri <= Math.floor(afterQuad / 3); numTri++) {
            ...
            if (afterTri >= 0 && afterTri % 2 === 0) {
              ...
              const totalWinners =
                winnersFromQuad + numTri * triWinners + num1v1;
...
function previewBrackets() {
  ...
  if (preview.length === 0) {
    elements.bracketPreview.innerHTML =
      '<p style="text-align:center;color:var(--color-text-muted)">Sin batallas (todos pasan)</p>';
    return;
  }

  elements.bracketPreview.innerHTML = preview
    .map((battle) => {
...
function startTournament() {
  ...
  const battles =
    state.previewBattles && state.previewBattles.length > 0
...
function renderBracket() {
  ...
  elements.bracketView.innerHTML = round.battles
    .map((battle, index) => {
      const statusClass = battle.completed
        ? "completed"
        : index === round.battles.findIndex((b) => !b.completed)
          ? "active"
          : "pending";
      const statusText = battle.completed
        ? "done"
        : statusClass === "active"
          ? "live"
          : "pending";
...
function openBattle(index) {
  ...
  // Setup modal
  elements.battleRoundLabel.textContent = battle.isWildcard
    ? "🎲 REPECHAJE"
    : `Batalla ${index + 1}`;
  ...
  // Render contestants
  elements.battleContestants.innerHTML = battle.contestants
    .map(
      (c, i) => `
...
function selectWinner(contestantIndex) {
  ...
  if (battle.size === 2) {
    ...
    const allContestantDivs =
      elements.battleContestants.querySelectorAll(".battle-contestant");
...
function confirmWinners() {
  ...
  if (selectedWinners.length !== battle.winnersNeeded) {
    alert(
      `Seleccioná ${battle.winnersNeeded} ganador${battle.winnersNeeded > 1 ? "es" : ""}`,
    );
    return;
  }
...
function editBattleResult(battleIndex) {
  ...
  if (state.tournament.rounds.length > state.currentRound + 1) {
    if (
      !confirm(
        "⚠️ Editar este resultado podría afectar las siguientes rondas. ¿Continuar?",
      )
    ) {
      return;
    }
...
function exportResults() {
  ...
  const champion = lastRound.battles.every((b) => b.completed)
    ? lastRound.battles.flatMap((b) => b.winners)[0]
    : null;
...
function showTournamentMenu() {
  ...
  menu.onclick = (e) => {
    if (e.target === menu) closeTournamentMenu();
  };
  menu.innerHTML = `
        <div class="modal-content battle-card" style="max-width: 320px;">
            <div class="battle-header">
...
function addLateMcToList() {
  ...
  document.getElementById("lateMcList").innerHTML = window.lateMcs
    .map(
      (n, i) => `
        <div class="mc-item">
